...
pub enum DiffKind {
    ...
    SubSelectionToLitObject { source_range: Option<(usize, usize)> },
    ...
    LegacyObjectToLitObject { source_range: Option<(usize, usize)> },
...
impl JSONSelection {
    ...
    /// `true`.
    ///
    /// This is intended for surveying how the *same* source text
    /// parses differently under different [`ConnectSpec`] versions;
    /// the comparison is not meaningful when applied to unrelated
    /// selections.
...
fn classify_top_level_key_to_literal(
    ...
    match (key, v4.as_ref()) {
        (Key::Field(name), LitExpr::Null) if name == "null" => {
            Some(DiffKind::KeyFlippedToLiteralNull { source_range: range, followed_by })
        }
        ...
        (Key::Field(name), LitExpr::String(_)) => {
            Some(DiffKind::KeyFieldFlippedToLiteralString {
                text: name.clone(),
                source_range: range,
                followed_by,
            })
...
fn diff_litexpr(v3: &WithRange<LitExpr>, v4: &WithRange<LitExpr>, out: &mut Vec<DiffKind>) {
    ...
    if let LitExpr::Path(path) = v3_inner {
        if let Some(key) = single_key_of_path(path) {
            match (key, v4_inner) {
                (Key::Field(name), LitExpr::Null) if name == "null" => {
    ...
    if let (LitExpr::Path(path), LitExpr::Object(obj4)) = (v3_inner, v4_inner) {
        if let Some(sub3) = path_starts_with_subselection_only(path) {
    ...
    if let (LitExpr::Path(path), LitExpr::LitPath(root, tail4)) = (v3_inner, v4_inner) {
        if let PathList::Key(key, rest3) = path.path.as_ref() {
            let range = range_to_pair(v4.range());
            ...
            match (key.as_ref(), root.as_ref()) {
                ...
                (Key::Field(name), LitExpr::Bool(value))
